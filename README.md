...
The implementation of this project only focus on the two files

``` 
QuadControl.cpp contains required moules for the controller 
QuadControlParams.txt contains the configuration files for controller and the vehicle
``` 

### 1. Scenario 1 Introduction ###

Tune the mass parameter in the QuadControlParams.txt file to keep the vertical forces equal and cause the drone to hover at its current altitude.

>Testing result
![Scenario1 TestResult](./misc/Scenario_1.PNG)
![Scenario1 GIF](./misc/scenario1_Pass.gif)

### 2. Scenario 2 Altitude Control ###
Implemented 3 methods in QuadControl.cpp
- ```BodyRateControl()```  -> line 91
- ```GenerateMotorCommands()``` ->  line 59
- ```RollPitchControl()``` -> line 124
Tuned kpPQR parameter in (QuadControlParams.txt) to prevent the drone from spinning to quickly 
Tune kpBank parameter in (QuadControlParams.txt) to minimize settling time but avoid too much overshoot

>Testing result
![Scenario2 TestResult](./misc/Scenario_2.PNG)
![Scenario2 GIF](./misc/scenario2_Pass.gif)

### 3. Scenario 3 Position Control ###

Implemented 3 methods in QuadControl.cpp
- ```LateralPositionControl()```  -> line 212
- ```AltitudeControl()``` ->  line 168
- ```YawControl()``` -> line 256
...
>Testing result
![Scenario3 TestResult](./misc/Scenario_3.PNG)
![Scenario3 GIF](./misc/scenario3_Pass.gif)
### 4. Scenario 4 Non-idealities ###
The configuration with 3 quads that are all are trying to move one meter forward 
...
![Scenario4 TestResult](./misc/Scenario_4.PNG)
![Scenario4 GIF](./misc/scenario4_Pass.gif)

			
### 5. Scenario 5 Trajectory Follow ###
 This scenario has two quads:
- the orange one is following traj/FigureEight.txt
- the other one is following traj/FigureEightFF.txt

>Testing result
![Scenario5 TestResult](./misc/Scenario_5.PNG)
![Scenario5 GIF](./misc/scenario5_Pass.gif)
